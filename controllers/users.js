...
export const signUp = async (req, res) => {
  try {
    const { name, email, password, task_list } = req.body
...
export const signIn = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email }).select(
      'name email password_digest task_list'
    )
...
export const addToTasks = async (req, res) => {
  try {
    const { id } = req.params
    const user = await User.findById(id)
    user.task_list.push(req.body)
...
export const removeFromTasks = async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
    user.task_list.splice(req.body.idx, 1)
    await user.save()
